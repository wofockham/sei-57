...
users["Joel"] = {:twitter => "edwardawkward", :favorite_numbers => [12, 777, 420]}
...
evens = users["Anil"][:favorite_numbers].select do |number|
    number.even?
end
p evens
...
# users.values.map { |user| user[:favorite_numbers] }.inject &:&
